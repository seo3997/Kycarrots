...
2. 사용자가 커밋 메시지를 입력하면 다음 명령을 수행합니다:
   ...
   ```
3. 푸시가 성공하면 즉시 "**GitHub 푸시가 완료되었습니다. 어떤 젠킨스 빌드를 실행할까요? (was / front / 모두)**"라고 묻습니다.
4. 사용자가 선택하면 해당하는 젠킨스 빌드를 API를 통해 즉시 실행합니다.
   - **WAS** (또는 wsa) 입력 시: `asagong-was` 빌드 실행
   - **front** 입력 시: `asagong-frontend-deploy` 빌드 실행
5. **빌드 결과 확인**: 빌드가 시작되면 AI는 빌드 번호를 추적하고, 빌드가 끝날 때까지 모니터링하여 **성공/실패 여부와 주요 로그 요약**을 사용자에게 보고합니다.

// turbo
#### 3. 젠킨스 빌드 대상 및 정보
1. **asagong-was** (WAS)
   - **URL**: `http://152.70.255.251:8080/job/asagong-was/build`
2. **asagong-frontend-deploy** (Frontend)
   - **URL**: `http://152.70.255.251:8080/job/asagong-frontend-deploy/build`

- **모니터링**: 빌드 완료 시까지 상태 체크 후 `consoleText` 요약 보고.
