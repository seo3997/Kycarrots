...
<head>
	...
	//<![CDATA[
		...
		function fnGoPage(currentPage){
			...
			$("#aform").attr({action:"/mgt/survey/userGroup.do", method:'post'}).submit();
		}

		// 상세조회
		function fnSelect(user_no){
			$("#user_no").val(user_no);
			$("#aform").attr({action:"/mgt/survey/userGroupMgt.do", method:'post'}).submit();
		}
		// 검색
		function fnSearch(){
			$("#currentPage").val("1");
								...
								<table class="table table-hover table-bordered">
									...
									%>
										<tr style="cursor:pointer;cursor:hand;" onclick="fnSelect('<%=dataMap.getString("USER_NO")%>')">
											...
											<td><%=dataMap.getString("EMAIL") %></td>
											<td class="text-center"><%=dataMap.getString("USER_STTUS_NM") %></td>
											<td class="text-center"><%=dataMap.getString("REGISTER_NM") %></td>
										</tr>
									<%  }%>
