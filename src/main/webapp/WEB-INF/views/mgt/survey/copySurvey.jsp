...
<% 
    ...
    String templateSurveyId=param.getString("templateSurveyId");
	String pageTitle = "Create New Survey";
    int minLenSurveyName=1;
	int maxLenSurveyName=30;
	boolean invalidSurveyName = false;
	
	String surveyName = StringUtil.shortenText("Copy of " + survey.getSURVEY_TITLE(), maxLenSurveyName - 3 );
	
	if (!param.getString("createSurvey").equals("")) {
					...
					<td>
						<%
						  if ( templateSurveyId != null ) {
						    out.println ( "The survey" + " &quot;" + survey.getSURVEY_TITLE() + "&quot; " + "is used as a template." + "<br>" );
						  }
						  if ( invalidSurveyName ) {
						    out.println ( "<font color=\"red\"><b>" + "The survey name contains invalid characters." +"</b></font><br>" );
						  }
						%>
						<form method="post" action="/mgt/survey/copySurvey_ok.do" name="form">
						<b>Survey name:</b>
						<font color="#999999">(used for your reference; users do not see the survey name)</font><br>
						<input type="text" name="surveyName" value="<%=surveyName%>" size="<%=maxLenSurveyName+10%>" maxlength="<%=maxLenSurveyName%>" tabindex="1">
						...
						%>
						<input type="submit" name="createSurvey" value="OK" class="btn btn-primary">&nbsp;&nbsp;<input type="button" name="cancel" value="Cancel" class="btn" onClick="self.close();">
						</form>
					</td>
				</tr>
				</tbody>
