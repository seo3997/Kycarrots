...
<html lang="ko">
					...
					<div class="widget-box">
						...
						<div class="widget-body">
							<div class="widget-body-inner" style="display: block;">
								<div class="widget-main">
									<div class="row-fluid">
										<div class="span4 graph-text-center alert-success">예약시간대비 30분초과 대기</div>
										<div class="span4 graph-text-center alert-info">대기시간 공지수준</div>
										<div class="span4 graph-text-center alert-error">대시사유 공지수준</div>
									</div>
									<div class="space"></div>
									<div class="row-fluid">
										<div class="span4">
										<div id="chartHolderDimAvg1" class="graph-faddind-left" style="height:200px;"></div>
										</div>
										<div class="span4">
										<div id="chartHolderDimAvg2" class="graph-faddind-left" style="height:200px;"></div>
										</div>
										<div class="span4">
										<div id="chartHolderDimAvg3" class="graph-faddind-left" style="height:200px;"></div>
										</div>
									</div>
									...
									<div class="row-fluid">
										<div class="span4">
										<div id="chartHolderDim1" class="graph-faddind-left" style="height:350px;"></div>
										</div>
										<div class="span4">
										<div id="chartHolderDim2" class="graph-faddind-left" style="height:350px;"></div>
										</div>
										<div class="span4">
										<div id="chartHolderDim3" class="graph-faddind-left" style="height:350px;"></div>
										</div>
									</div>
	...
	<script>
	     ...
	     $("#statistics_cd").on("change",function(e){
	 		if ($(this).val() =="1")	{
	 			location.href="/mgt/survey/statisticsEntry.do?surveyId=<%=surveyId%>&userNo=<%=param.getString("userNo")%>&surveyGroupId=<%=param.getString("surveyGroupId")%>&statisticsCd=1";
	 		}else if ($(this).val() =="2")	{
	 			location.href="/mgt/survey/statisticsCd1Entry.do?surveyId=<%=surveyId%>&userNo=<%=param.getString("userNo")%>&surveyGroupId=<%=param.getString("surveyGroupId")%>&statisticsCd=2";
	 		}else if ($(this).val() =="3")	{
	 			location.href="/mgt/survey/statisticsCd2Entry.do?surveyId=<%=surveyId%>&userNo=<%=param.getString("userNo")%>&surveyGroupId=<%=param.getString("surveyGroupId")%>&statisticsCd=3";
	 		}else if ($(this).val() =="4")	{
	 			location.href="/mgt/survey/statisticsCd2Entry.do?surveyId=<%=surveyId%>&userNo=<%=param.getString("userNo")%>&surveyGroupId=<%=param.getString("surveyGroupId")%>&statisticsCd=4";
	 		}else if ($(this).val() =="5")	{
	 			location.href="/mgt/survey/statisticsCd2Entry.do?surveyId=<%=surveyId%>&userNo=<%=param.getString("userNo")%>&surveyGroupId=<%=param.getString("surveyGroupId")%>&statisticsCd=5";
	 		}
	 	});
		            ...
		            +'<series>'
		              ...
		              +'<Column2DSet type="stacked" showTotalLabel="true" totalLabelJsFunction="totalFunc">'
		               ...
		               /*  Column2D Stacked 를 생성시에는 Column2DSeries를 최소 2개 정의합니다 */
		                     +'<Column2DSeries labelPosition="inside" yField="GRADE_04_VAL" displayName="강한부정"  color="#ffffff" strokeJsFunction="strokeFunction">'
		                           +'<showDataEffect>'
		                               +'<SeriesInterpolate/>'
		                           +'</showDataEffect>'
		                      +'</Column2DSeries>'
		                      +'<Column2DSeries labelPosition="inside" yField="GRADE_03_VAL" displayName="부정"  color="#ffffff" strokeJsFunction="strokeFunction">'
		                           +'<showDataEffect>'
		                               +'<SeriesInterpolate/>'
		                           +'</showDataEffect>'
		                      +'</Column2DSeries>'
		                      +'<Column2DSeries labelPosition="inside" yField="GRADE_02_VAL" displayName="긍정"  color="#ffffff" strokeJsFunction="strokeFunction">'
	                         +'<showDataEffect>'
	                            +'<SeriesInterpolate/>'
	                         +'</showDataEffect>'
			                    +'</Column2DSeries>'
			                    +'<Column2DSeries labelPosition="inside" yField="GRADE_01_VAL" displayName="강한긍정"  color="#ffffff" strokeJsFunction="strokeFunction">'
			                    +'<showDataEffect>'
			                       +'<SeriesInterpolate/>'
			                   +'</showDataEffect>'
			                  +'</Column2DSeries>'
		    	...
		    	for(int j=0;j<tbSatisfactionItem.size();j++){
		    		TbSatisfactionItem aTbSatisfactionItem = tbSatisfactionItem.get(j);
		  			...
		  			for(int k=0;k<aSatisfactionItemVals.size();k++){
			  			...
			  			RateJSON.put("YEARDEGREE", aTbSurveyUserEntryResult.getSURVEY_YEAR()+aTbSurveyUserEntryResult.getSURVEY_DEGREE());
