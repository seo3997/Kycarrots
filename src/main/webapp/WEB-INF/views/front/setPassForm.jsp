...
<html lang="en">
		...
		<script type="text/javascript">
					 	...
					 	var results=pResponse.resultStats.resultList;
		               	if(pResponse.resultStats.resultCode=="ok"){
		               		if(pResponse.resultMap == 'Y'){
								...
								$("#spDangerAlert").html("이메일 검증 되었습니다.");
								$("#idcheck").val("Y");
								return;
							}else{
								$("#idcheck").val("N");
								...
								$("#spSuccessAlert").html("이메일이 없습니다. 이메일을 확인하세요");
							}
		               	}
				 });
			}
			...
			function fnGoInsert(){
				...
				if(bool){
					...
					if(confirm("패스워드를 재설정 하시겠습니까?")){
						   var sUrl	  = "/front/setPassAjax.do";
			               var fFrom  =$("#aform");
							fnCallbackFormAjax(sUrl,fFrom,function fncallback(pResponse){
								console.log(pResponse);
			                	if(pResponse.resultStats.resultCode=="ok"){
			                   		if(pResponse.resultMap == 'Y'){
				                		alert("패스워드 재설정이 완료 되었습니다.");
				        				$("#mySuccessAlert").show("fast");
				        				$("#spSuccessAlert").html("패스워드 설정이 완료 되었습니다.로그인후 사용하세요.");
			    					}else{
				                		//alert("처리중 오류가 생겼습니다 \N 저장버튼을 다시 클릭하세요.");
				        				$("#myDangerAlertS").show("slow").delay(2000).hide("3000");
				        				$("#spDangerAlert").html("처리중 오류가 생겼습니다. 패스워드 재설정버튼을 다시 클릭하세요.");
	...
	<section id="contain">
		<div class="container">
		  <div class="row">
			<div class="span8">
				<h4 class="heading">패스워드 재설정<span></span></h4>
				...
				<input type="hidden" id="idcheck"  name="idcheck" value="N"/>
					<fieldset>
						...
						<input type="email" name="user_id" id="user_id" maxlength="50" value="" class="span5" placeholder="Enter your unique email address." />
							<input  class="btn btn-dpgray" type="button" id="btn_idcheck" style="margin-top: -10px;" value="패스워드 검증" />
						</p>
					</fieldset>
					<fieldset>
						<p>
					...
					<fieldset>
						<p>
						<input class="btn btn-large btn-primary" type="button" id="btn_save" value="패스워드 재설정"/>
						</p>
					</fieldset>	
				</form>
